...
class leaveApplyForm(ModelForm):
    class Meta:
        ...
        widgets = {
            'leave_date': DateInput(attrs={'type': 'date','id': 'leave_date_input'})
...
class loanApplyForm(ModelForm):
    class Meta:
        model = TLoan
        fields = ['loan_amount','loan_date','loan_reason','loan_period']
        widgets = {
            'loan_date': DateInput(attrs={'type': 'date'})

        }
    def __init__(self, *args, **kwargs):
        super(loanApplyForm, self).__init__(*args, **kwargs)
        for visible in self.visible_fields():
            visible.field.widget.attrs['class'] = 'form-control'
